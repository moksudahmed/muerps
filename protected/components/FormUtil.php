...
class FormUtil 
{
    ...
    public static function countGenderNewlyAdmittedStudent($programCode,$year,$term=null)
    {
        $student = new Student();
   ...
   public static function checkItemExists($group_name, $start, $end, $rows=array()){
    ...
    public static function getSuppleExamType()
    {
        //$month= parent::formatMonth('M', CTimestamp::getDate());
    ...
    public static function tableDropDown()
    {
        
        //$string = array();
        $string = array('tbl_a_school','tbl_aa_fees','tbl_aa_fees','tbl_ab_waiver','tbl_ac_batchfees',
        'tbl_ad_options','tbl_b_department','tbl_c_programme','tbl_d_syllabus','tbl_e_module','tbl_f_batch',
        'tbl_g_section','tbl_h_offeredmodule','tbl_i_administration','tbl_j_person','tbl_k_academichistory',
        'tbl_k_institution');
         ...
         public static function getTermYearForTranscript($batchName,$programmeCode,$term, $year)
    {
        ...
        public static function getFacultyByUser()
        {
            /*   return array(
                        array('id'=>256,'text'=>'TV'.'ok','group'=>'Electrical'),
                        array('id'=>257,'text'=>'Radio','group'=>'Electrical'),
                    ...
                    WHERE 
                      p.\"personID\" = f.\"facultyID\" AND
                      u.\"personID\" = p.\"personID\" AND
                      f.\"departmentID\" = d.\"departmentID\" AND 
                      f.ex_fac_active=true AND
                      u.usr_active = true
                      order by d.dpt_code, p.\"per_firstName\"
        ...
        public static function getRoom()
        {
            ...
            public static function getDepartmentByID($id,$type)
            {
               
               if($type=='e')
               { 
                   $adm = Administration::model()->findByPk($id);
  ...
  //echo $sql;  exit();
        ...
        public static function getBatchByProgramme($id)
        {
          ...
          public static function getExamType($id)
          {
              ...
              if ($exam->exm_type ==1) return 'Final';
              elseif ($exam->exm_type ==2) return 'Supplementary';
              elseif ($exam->exm_type ==3)return 'Special Supplementary';
          }
          
          public static function getExamName($exam=1)
          {
              
              if ($exam == 1) return 'Final';
              elseif ($exam == 2) return 'Supplementary';
              elseif ($exam == 3) return 'Special Supplementary';
          }
      ...
      //                  =>':regID','emr_mark'=>':mrk'),array(':regID'=>$modRegID));
         ...
         public static function getOptionDepartment()
        {
            ...
            $sql = "SELECT DISTINCT \"departmentID\", \"dpt_name\"
            ...
            foreach ($rows as $row)
            {
                    ...
                    $data[$row['departmentID']]= $row['dpt_name']; //array('group'=>$itme->option_group);                       
                   // $data[$i]= array('group'=>$row['option_group']);                       
...
//                                          $param =array ("programmeCode"=> (string)yii::app()->session['reProCode'],"batchName" => (integer)yii::app()->session['reBatName'],"examinationID" => (integer) yii::app()->session['examinationID'], "mod_group" => yii::app()->session['group']);                                                                                            
     ...
     public static function getOldSyllabusModule($id, $secName, $batchName, $programeCode)
    {
        ...
        public static function getSuperAdminFlag($user,$term,$year)
        {
           // echo $user.$term.$year;
           // exit();
            if (strtolower($user)=='super-admin')
            {
                return TRUE;
            }
            elseif (strtolower($user)=='deo')
            {
                return TRUE;
            }
            else{
                if($term==yii::app()->session['MainCurTerm'] && $year==yii::app()->session['MainCurYear'])
                    return TRUE;
                else                    
                    return FALSE;
            }
        ...
        public static function sendMail($to,$from,$fromName,$subject,$message){
        ...
        public static function getActiveDeactive($flag)
        {
        ...
        public static function getAccessRole($userType)
        {
            /*   return array(
                        array('id'=>256,'text'=>'TV'.'ok','group'=>'Electrical'),
                        array('id'=>257,'text'=>'Radio','group'=>'Electrical'),
                 ...
                 if($userType==1)
                 {
                     $data[0]= array('id'=>'head','text'=>'Head');
                     $data[1]= array('id'=>'coordinator','text'=>'Coordinator');
                     $data[2]= array('id'=>'faculty','text'=>'Faculty');
                     $data[3]= array('id'=>'super-admin','text'=>'Super-Admin');
                     //echo CHtml::listData($item2, 'programmeCode', 'pro_name','departmentID');
                 }
                 else
                 {
                     $data[0]= array('id'=>'admin','text'=>'Admin');
                     $data[1]= array('id'=>'admission','text'=>'Admission');
                    
                     $data[2]= array('id'=>'deo','text'=>'DEO');
                     $data[3]= array('id'=>'exam','text'=>'Exam');                        
                     $data[4]= array('id'=>'registry','text'=>'Registry');
                     $data[5]= array('id'=>'super-admin','text'=>'Super-Admin');
                 }
        ...
        public static function getAllExamType()
        {
               return array(
                        array('group'=>'Term Final'),
                        array('group'=>'Supplementary'),
                        array('group'=>'Special Supplementary'),
                );
