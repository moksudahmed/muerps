...
class TermAdmission extends CActiveRecord
{
    ...
    public function searchTermAdmissionByID($sid)
    {
        
        $criteria=new CDbCriteria;
        
		$criteria=new CDbCriteria;
        $criteria->select="\"termAdmissionID\",\"sectionName\", \"batchName\",               
        tra_term, tra_year, tra_date, \"tra_finalExamRegistred\", 
       \"tra_finalExamRegDate\" ";
        
        
        
        $criteria->condition='t."studentID"=:sid';
    ...
    public function searchExamRegisteredSuppleList($proCode,$traTerm,$traYear)
    {
      ...
      //          $criteria->params=array(':proCode'=>$proCode);
        ...
        public static function searchNotTermAdmittedStudent($secName,$batName,$proCode,$traTerm,$traYear)
	{
                    
            
            
                      $sql ="
                         select \"studentID\" as ID, \"studentID\", pg_catalog.concat(\"per_firstName\", ' ', \"per_lastName\") AS per_name,
                         per_mobile, \"per_bloodGroup\"  from 
                        ( SELECT DISTINCT  s.\"studentID\" as ID   FROM (
                        SELECT 
                          \"studentID\"
                        FROM 
                          tbl_p_admission p
                      ...
                      \"batchName\" = {$batName} AND 
                      tra_term = {$traTerm} AND 
                      tra_year = {$traYear} AND 
                      \"programmeCode\" = '{$proCode}') ) as p, tbl_j_person as j, tbl_o_student as o where p.ID = o.\"studentID\" 
                          and j.\"personID\" = o.\"personID\" order by  o.\"studentID\"
                        ";
