...
/* @var $model Module */
            ...
            $this->breadcrumbs=array(
                ...
                'Special Retake'=>array('headsFunction/GetRegisteredModuleForSpecialRetake'),
                'Marks Entry'
                );
...
$secondHalfMarks= $markingScheme->mrs_final;
    ...
    <div class="title ">
        <div class="span-16">
            ...
            <h4> <?php echo FormUtil::getBatchTermHTMLspan(yii::app()->session['mestSection'],yii::app()->session['mestBatch'],yii::app()->session['mestProCode'] ); ?></h4>
            <h4><strong>Course: </strong><span class="label label-success"><?php echo yii::app()->session['mestModule']; ?></span></h4>
            <h4><strong>Faculty Name: </strong><span class="label label-info"><?php echo yii::app()->session['mestFacultyName']; ?></span></h4>
            
            
              
        </div>
        <div class="span-7">
            <h4><strong>Term: </strong><span class="label label-warning"> <?php echo FormUtil::getTerm(yii::app()->session['mestTerm']); ?>  </span><strong>Year: </strong><span class="label label-info"> <?php echo yii::app()->session['mestYear']; ?></span></h4>
            <h6>Programme:  <?php  echo DBhelper::getProgrammeByCode(yii::app()->session['mestProCode']); ?></h6>
            <h6> <?php echo FormUtil::getTerm( yii::app()->session['mestTerm']);?> <?php echo "Term Final Examination ".yii::app()->session['mestYear'];?></h6>
<?php 


    $backUrl=Yii::app()->controller->createUrl('headsFunction/GetRegisteredModuleForSpecialRetake');
    $backTitle='Special Retake';
    ...
    <?php  if ($moduleReg !== null):?>
                    ...
                    <?php $form = $this->beginWidget(
                        'bootstrap.widgets.TbActiveForm',
                            ...
                            'action'=>CController::createUrl('saveTotalMarksSpecialRetake'),
                        )
                            );
                    ?>
...
<table class="table table-bordered span-24">
	...
	<tr>
        ...
        if(totalmark>=79.5 && totalmark<=100){ return 'A+';}
	else if(totalmark>=74.5 && totalmark<79.5){return 'A';}
	else if (totalmark>=69.5 && totalmark<74.5){ 
		return 'A-';}
	else if (totalmark>=64.5 && totalmark<69.5) {
		return 'B+';}
	else if (totalmark>=59.5 && totalmark<64.5) {
		return 'B';}
	else if (totalmark>=54.5 && totalmark<59.5) {
		return 'B-';}
	else if (totalmark>=49.5 && totalmark<54.5) {
		return 'C+';}
	else if (totalmark>=44.5 && totalmark<49.5) {
		return 'C';}
	else if (totalmark>=39.5 && totalmark<44.5) {
		return 'D';}
	else { return 'F*';}
