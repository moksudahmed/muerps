...
?>
    ...
    <div class="title span-24">
        <div class="span-14">
            <h3 >Marks Entry For: [ Old Student ]</h3>
		...
		<th >
 		...
 		<!--th >
		      <span style="padding: 0px 3px 0px 0px;"> ( Attendance<sup><?php echo $markingScheme->mrs_attendance; ?></sup> - Class Test<sup><?php echo $markingScheme->mrs_classTest; ?></sup> - Midterm<sup><?php echo $markingScheme->mrs_midterm; ?></sup> )</span>
                ...
                <th>
                    <span style="font-size: 15px; padding: 0px 17px 0px 52px;"> 60+40 <!---sup> <?php echo $secondHalfMarks; ?> </sup--></span>
                ...
                <td class="span-8">
                    <div id="form-<?php echo $i; ?>" >
                       <?php $sql = "select * from vw_getsecondhalfmarkslist where \"moduleRegistrationID\"=:regID and \"examinationID\"=:examID ;";
                        $regOldStu= ModuleRegistration::model()->findBySql($sql,array(':regID'=>$row['moduleRegistrationID'],':examID'=>yii::app()->session['mreExaminationID'])); ?>
                       <?php $this->renderPartial('_formOldStudent',array('regOldStu'=>$regOldStu,'i'=>$i)); ?>
                    </div> 
