...
class PersonController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','addreferee','FreeSearch','searchEngine','SearchEngineAjax'),
				'users'=>array('@'),
			),
	...
	protected function performAjaxValidation($model)
	{
        ...
        //  echo "THIS IS IAJAXX  ".$keyword;
            ...
            $keyword = explode (' ',$request->getPost('studentID'));
             
            if($keyword[0]!=null)
            {
                yii::app()->session['searchKey']=$keyword[0];
            }
           // echo $keyword[0];
            
            $model=new Admission();
            //$model->unsetAttributes();  // clear any default values
            $dataProvider=$model->freeSearch(yii::app()->session['searchKey']);
            $this->render('search',array(
                    'dataProvider'=>$dataProvider
            ));
        }
        
        public function actionSearchEngineAjax()//$keyword
        {
        //  echo "THIS IS IAJAXX  ".$keyword;
