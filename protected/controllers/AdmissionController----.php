...
class AdmissionController extends Controller
{
	...
	public function accessRules()
	{
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('admin','delete','update','reAdmission','CreateReAdmission','SectionTransfer','ChangeSection','AdmissionRegister','studentsInfo'),
				'users'=>array(yii::app()->user->id),
			),
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('admin','delete','update','reAdmission','CreateReAdmission','SectionTransfer','ChangeSection','AdmissionRegister'),
				'users'=>array(yii::app()->user->id),
			),
			array('deny',  // deny all users
				'users'=>array('*'),
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('admin','delete','update','reAdmission','CreateReAdmission','SectionTransfer','CreateSectionTransfer','AdmissionRegister'),
				'users'=>array(yii::app()->user->id),
			),
			array('deny',  // deny all users
				'users'=>array('*'),
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				//'actions'=>array('studentsInfo','update'),
                                'actions'=>array('studentsInfo','update','reAdmission','studentsInfo','CreateReAdmission','SectionTransfer','CreateSectionTransfer'),
				'users'=>array(yii::app()->user->id),
			),
			array('deny',  // deny all users
				'users'=>array('*'),
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('index','getBatch','getSection','admissionReport','admissionReport_programm','getBatchByProgrammeCode','getSectionByProgrammeCode','reAdmission','CreateReAdmission','reTake','CreateTermAdmissionForRetake','SaveRetake','DeleteRetake'),
				'users'=>array(yii::app()->user->id),
			),
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
 ...
 {$admission->waiverID},
        ...
        public function actionCreateReAdmission()
	{
                ...
                {
                    $tad=  TermAdmission::model()->findAllByAttributes(array('studentID'=>$_REQUEST['studentID'],'tra_term'=>$_REQUEST['startTerm'],'tra_year'=>$_REQUEST['startYear']));
                    if(count($tad)){
                       Yii::app()->user->setFlash('warning','Re-admission failed of ID:'.$_REQUEST['studentID'].', because this ID has already been admitted in this term with the current batch !!');
                        $this->redirect(array('studentsInfo'));                 
                    }
                    ...
                    if($_REQUEST['waiverID']!=null)
                    {
                        $sql2= " INSERT INTO tbl_p_admission(
                        \"studentID\", \"sectionName\", \"batchName\", \"programmeCode\", \"adm_startTerm\",\"adm_startYear\",  
                         \"adm_expireTerm\", \"adm_expireYear\",adm_remarks, ex_adm_active, adm_status, \"waiverID\")
                        VALUES ('{$_REQUEST['studentID']}','{$split[0]}',{$split[1]},'{$split[2]}',{$_REQUEST['startTerm']},{$_REQUEST['startYear']},{$expireTerm},{$expireYear},'{$_REQUEST['remarks']}',true,{$_REQUEST['adm_status']},{$_REQUEST['waiverID']});";
                    }
                    else
                    {
                        $sql2= " INSERT INTO tbl_p_admission(
